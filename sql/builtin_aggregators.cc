...
struct StringAggAccum {
    ...
    void process(const ExpressionValue * args, size_t nargs)
    {
        ...
        cerr << "process " << val.toUtf8String()
             << " with separator " << separator.toUtf8String() << endl;
        cerr << "existing is '" << value << "' first " << first << endl;
        ...
        cerr << "value is '" << value << "' first " << first << endl;
    ...
    {
        cerr << "merge of " << value << " and " << src->value << endl;
        if (src->first)
